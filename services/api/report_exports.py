...
def create_export(store: RunStore, run_id: str, revision: int, chart_svg: Callable[[dict[str, Any]], str]) -> tuple[str, dict[str, Any]]:
    ...
    export_id = str(uuid4())
    html_text = render_report_html(document, chart_svg)
    manifest = {
        "export_id": export_id,
        "run_id": run_id,
