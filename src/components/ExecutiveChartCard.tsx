...
export default function ExecutiveChartCard({ instance, profile, language, onPin, onViewRecords }: Props) {
 ...
 const hasFewerCategories = Boolean(result && !dimensionIsTime && result.result_count < request.limit);
 const visibleTitle = hasFewerCategories ? (result?.title ?? instance.title).replace(/^Top \d+/, `Top ${request.limit}`) : (result?.title ?? instance.title);
 const showCompanionTable = Boolean(result && (request.chart_type === 'pie' || request.chart_type === 'donut'));
 const total = result?.rows.reduce((sum, row) => sum + Math.max(0, Number(row.value)), 0) ?? 0;
 return <article className={`executive-chart-card role-${instance.role}`}><header className="chart-card-header"><div><p className="section-eyebrow">{instance.role === 'trend' ? text(language,'primaryTrend') : text(language,'customChart')}</p><h2>{visibleTitle}</h2></div>{loading && <span className="chart-status"><LoaderCircle size={14} className="animate-spin"/>{text(language,'updating')}</span>}</header><div className="chart-controls"><label>{text(language,'metric')}<select value={request.metric} onChange={e => updateRequest({...request, metric:e.target.value})}>{metrics.map(x=><option key={x.name}>{x.name}</option>)}</select></label><label>{text(language,'dimension')}<select value={request.dimension} onChange={e => updateRequest({...request, dimension:e.target.value})}>{dimensions.map(x=><option key={x.name}>{x.name}</option>)}</select></label><label>{text(language,'type')}<ChartTypeSelect value={types.includes(request.chart_type) ? request.chart_type : types[0]} options={types} onChange={updateType} language={language}/></label>{request.chart_type === 'combo' && <label>{language === 'vi' ? 'Chỉ số đường' : 'Line metric'}<select value={request.secondary_metric ?? ''} onChange={e => updateRequest({...request, secondary_metric:e.target.value})}>{metrics.filter(x => x.name !== request.metric).map(x=><option key={x.name}>{x.name}</option>)}</select></label>}<label>{limitName}<select value={selectedLimit} onChange={e => { const custom = e.target.value === 'custom'; setCustomLimitMode(custom); updateRequest({...request, limit: custom ? boundedLimit(request.limit) : boundedLimit(Number(e.target.value))}); }}><option value={5}>{limitName} 5</option><option value={10}>{limitName} 10</option><option value={12}>{limitName} 12</option><option value={20}>{limitName} 20</option><option value={30}>{limitName} 30</option><option value="custom">Custom (1–30)</option></select>{customLimitMode && <input aria-label={`Custom ${limitName}`} type="number" min="1" max="30" step="1" value={request.limit} onChange={e => updateRequest({...request, limit: boundedLimit(Number(e.target.value))})}/>}</label></div>{error && <p className="chart-error"><TriangleAlert size={16}/>{error}</p>}<div className={showCompanionTable ? 'chart-detail-grid' : undefined}><div className="chart-visual">{result ? <ValidatedChart result={result} language={language}/> : <p>{text(language,'chartConfigure')}</p>}</div>{showCompanionTable && result && <section className="chart-data-table" aria-label={`${result.metric_display_name ?? result.metric} by ${request.dimension}`}><div><p className="section-eyebrow">Data table</p><b>{result.metric_display_name ?? result.metric} by {request.dimension}</b></div><div className="chart-data-table-scroll"><table><thead><tr><th>#</th><th>{request.dimension}</th><th>{result.metric_display_name ?? result.metric}</th><th>%</th></tr></thead><tbody>{result.rows.map((row, index) => <tr key={row.label}><td>{index + 1}</td><td title={row.display_label ?? row.label}>{row.display_label ?? row.label}</td><td>{formatChartValue(Number(row.value), language)}</td><td>{total ? `${(Number(row.value) / total * 100).toFixed(1)}%` : '—'}</td></tr>)}</tbody></table></div></section>}</div>{hasFewerCategories && <p className="chart-availability">{language === 'vi' ? `${result?.result_count} category hợp lệ trong phạm vi hiện tại.` : `${result?.result_count} categories available in the current scope.`}</p>}{result && <footer className="chart-actions"><button disabled={loading || pinning} onClick={pin}>{pinning ? text(language,'saving') : text(language,'addReport')}</button>{onViewRecords && <button disabled={loading} onClick={viewRecords}>{text(language,'viewRecords')}</button>}</footer>}</article>;
}
