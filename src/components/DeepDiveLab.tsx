...
import { ChevronDown, ChevronLeft, ChevronRight, Download, LoaderCircle, Plus, Search, SlidersHorizontal, TriangleAlert, X } from 'lucide-react';
import { DataFilter, DataQueryResult, DatasetProfile, EDAResult } from '../types';
...
export default function DeepDiveLab({ profile, eda, language }: { profile: DatasetProfile; eda: EDAResult | null; language: Language }) {
  ...
  useEffect(() => { setFilterColumn(fields[0]?.name ?? ''); }, [profile.run_id]);
  ...
  return <section className="max-w-[1400px] mx-auto space-y-4">
    ...
    <div className="data-explorer"><div className="data-toolbar"><label className="data-search"><Search className="w-4 h-4" /><span className="sr-only">Search visible data</span><input value={search} onChange={event => setSearch(event.target.value)} placeholder="Search visible data…" /></label><span className="result-count" aria-live="polite">{loading ? <LoaderCircle className="w-3.5 h-3.5 animate-spin" /> : null}{resultLabel}</span></div>
      ...
      <div className="data-table-wrap">{!data && loading ? <div className="lab-state"><LoaderCircle className="w-5 h-5 animate-spin" />Loading safe data…</div> : data && data.rows.length ? <table className="data-table"><thead><tr>{data.columns.map(column => <th key={column.name}><button type="button" onClick={() => toggleSort(column.name)} aria-label={`Sort by ${column.display_name}`}>{column.display_name}<span className={sortBy === column.name ? 'sort-active' : ''}>{sortBy === column.name ? (sortDirection === 'asc' ? '↑' : '↓') : '↕'}</span></button></th>)}</tr></thead><tbody>{data.rows.map((row, index) => <tr key={`${page}-${index}`}>{data.columns.map(column => <td key={column.name}>{row[column.name] || '—'}</td>)}</tr>)}</tbody></table> : <div className="lab-state"><Search className="w-6 h-6 text-slate-300" /><b>No matching rows</b><p>Try changing the search or removing one of the filters.</p>{(search || filters.length) ? <button type="button" onClick={() => { setSearch(''); setFilters([]); }}>Clear explorer</button> : null}</div>}</div>
