...
import { artifactFor, blockFor, defaultAppearance, emptyReportV2, placementFor, richTextFor, withRichText, type ReportBlock, type ReportBlockAppearance, type ReportDocumentV2, type RichTextDocument } from './model';
...
const appearanceClass = (appearance?: ReportBlockAppearance) => `report-appearance-bg-${appearance?.background ?? 'default'} report-appearance-fg-${appearance?.foreground ?? 'default'}`;
...
export default function ReportBuilder({ runId, language }: Props) {
  ...
  const updateAppearance = (partial: Partial<ReportBlockAppearance>) => {
    if (!selected) return;
    const previous = selected.appearance ?? defaultAppearance;
    const background = partial.background ?? previous.background;
    const tone = background === 'default' ? undefined : background.replace('-pastel', '');
    let foreground = partial.foreground ?? previous.foreground;
    if (tone && foreground !== 'default' && foreground !== `${tone}-strong`) foreground = `${tone}-strong` as ReportBlockAppearance['foreground'];
    updateSelected(block => ({ ...block, appearance: { background, foreground } } as ReportBlock));
  };
